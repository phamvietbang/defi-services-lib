...
class VenusStateService(CreamStateService):
    ...
    def get_token_deposit_borrow_balance(
        ...
        for token in reserves_info:
            ...
            get_ctoken_decimals = f"decimals_{value['cToken']}_{block_number}".lower()
            symbol = decoded_data[get_symbol_id]
            decimals = decoded_data[get_decimals_id]
            ctoken_decimals = decoded_data[get_ctoken_decimals]
